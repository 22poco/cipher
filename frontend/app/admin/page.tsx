...
function ReviewPanel({
  ...
  return (
    <section className="grid gap-5 rounded-md border border-slate-200 bg-white p-5">
      ...
      <details className="rounded-md border border-slate-200 p-4">
        <summary className="cursor-pointer font-semibold text-slate-950">
          mock exam results ({dashboard.mock_exam_attempts.length})
        </summary>
        <div className="mt-4 grid gap-2">
          {dashboard.mock_exam_attempts.length > 0 ? (
            dashboard.mock_exam_attempts.map((attempt) => (
              <div
                key={attempt.id}
            ...
            <p className="text-sm text-slate-500">no mock exam attempts yet.</p>
          )}
        </div>
      </details>
    </section>
